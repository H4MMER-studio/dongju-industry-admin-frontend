...
import { ICertificationList, ICertificationForm, ActionType } from "@/interfaces";
...
export function* createCertificationSaga({ payload }: ActionType & { payload: ICertificationForm }) {
    ...
    if (payload.certification_start_date) {
        formData.append("certification_start_date", payload.certification_start_date);
    }
    ...
    try {
        yield call(API.POST, `${CERTIFICATION_API}`, { bodyData: formData }, "form");
...
export function* deleteCertificationSaga({ payload }: ActionType & { payload: { id: string } }) {
    try {
...
export function* watchCertificationList() {
    yield takeEvery(certificationActions.getCertificationList, getCertificationListSaga);
}

export function* watchCreateCertification() {
    yield takeEvery(certificationActions.createCertification, createCertificationSaga);
}

export function* watchDeleteCertification() {
    yield takeEvery(certificationActions.deleteCertification, deleteCertificationSaga);
}

export default [watchCertificationList, watchCreateCertification, watchDeleteCertification].map((fn) => fn());
